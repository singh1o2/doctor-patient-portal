...
function Card(props)
{
    return(
        <div className='doctor-card'>
            ...
            {props.doctor.status === 'Y' && <Link  className='card-item card-button' to= {`/chat?room=${props.doctor.email}&name=client`} ><BsChatLeftText style={{marginTop:'5px'}} fontSize = '1.3rem'/></Link>}
        </div>
    )
}